...
const InputDate = (props) => {
  ...
  return (
    <TouchableOpacity onPress={() => {
      ...
      {selected ? (<PoppinsText style={{ position: 'absolute', left: 20, color: '#000000' }} content={moment(date).format('DD/MM/YYYY')}></PoppinsText>) : (<PoppinsText style={{ position: 'absolute', left: 20 , color:"#808080"}} content={data === null ? "N/A" : data}></PoppinsText>)

      }
      <View style={{ position: "absolute", right: 10 }}>
        <DateIcon name="date-range" color="#000000" size={30}></DateIcon>
      </View>
      <DatePicker
        style={{ color: "#000000" }}
        modal
